...
// var audio = new Audio("./MusicGame.wav");
// audio.play().load;
...
// stopAudio.addEventListener("click", audio.pause());
...
function setup() {
  const canvas = createCanvas(600, 600); //tableau 4x4
  noLoop();
  grid = [
    [0, 0, 0, 0],
    [0, 0, 0, 0],
...
function addNumber() {
  ...
  for (let i = 0; i < 6; i++) {
    for (let j = 0; j < 6; j++) {
      if (this.grid[i][j] === 0) {
        options.push({ x: i, y: j }); // valid spot option => case ne comprend pas de chiffre
      }
    }
  }
